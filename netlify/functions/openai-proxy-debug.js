...
`;
  ...
  try {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        mensaje: "Respuesta cruda recibida desde OpenAI",
        contenido: raw
      })
    };
  } catch (err) {
    console.error("❌ Error al depurar OpenAI:", err.message);
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Error depurando OpenAI", details: err.message })
    };
  }
};
